...
public class MyMath2  {
    ...
    public static int staticNum; //static 다른메모리영역에 들어감, static붙는순간 공간이 하나 할당 //객체생성과 상관없 new와 별개
                    //static 붙은 변수는 이미 메모리영역에 올라감
    void instanceAbs() {
        System.out.println(num < 0 ? -num : num);
    }
    public static void staticAbs(int num) {
        System.out.println(num < 0 ? -num : num);
    }
    ...
    void print2() { //method 앞에 final 붙으면 오버라이딩 금지 부모꺼 그대로 써야됨
